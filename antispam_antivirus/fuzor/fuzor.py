...
class FuzorReport(ScannerPlugin):
    ...
    def _init_backend(self):
        ...
        red=redis.StrictRedis(host=host,port=port,db=int(db),socket_timeout=self.config.getint(self.section,'timeout'))
    ...
    def examine(self,suspect):
        ...
        try:
            self.logger.info("DEBUG: suspect %s to=%s hash %s usable_body=%s predigest=%s subject=%s"%(suspect.id,suspect.to_address,digest,hash.bodytext_size,hash.predigest[:50],suspect.get_message_rep().get('Subject')))
        except:
            pass
...
class FuzorCheck(ScannerPlugin):
    ...
    def _writeheader(self,suspect,header,value):
        ...
        if type(tag)==types.ListType:
            tag.append(hdr)
        elif type(tag)==types.NoneType:
            tag=[hdr,]
        else: #str/unicode
            tag="%s\r\n%s"%(tag,hdr)
    ...
    def examine(self,suspect):
        ...
        if digest!=None:
            ...
            if count>0:
                ...
                self.logger.info("digest %s from %s to %s seen %s times"%(digest,suspect.from_address,suspect.to_address,count))
        else:
            suspect.debug('Fuzor: not enough data for a unique digest')

        #diff=time.time()-start
        #self.logger.info("%s: FUZOR END (NORMAL), time = %.4f"%(suspect.id,diff))
        return DUNNO
...
class FuzorDigest(object):
    def __init__(self,msg):
        ...
        self.REMOVE_HTML_TAGS=['script','style'] # strip tags (including content)
    ...
    def _handle_text_part(self,part):
        ...
        if self.STRIP_HTML_MARKUP:
            payload=self.filter.strip_text(payload,remove_tags=self.REMOVE_HTML_TAGS,use_bfs=True)

        if self.REPLACE_EMAIL!=None:
            payload=re.sub(r'\S{1,50}@\S{1,30}',self.REPLACE_EMAIL,payload)

        if self.REPLACE_URL!=None:
            payload=re.sub(r'[a-z]+:\S{1,100}',self.REPLACE_URL,payload)

        if self.REPLACE_LONG_WORD!=None:
